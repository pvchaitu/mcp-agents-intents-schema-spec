# sqllite-openmcp-tests-v0.3.py (Updated Version - FIXES '_session' ERROR)
...
# ... (All previous CONFIGURATION code remains UNCHANGED) ...
...
# ... (initialize_mock_database and check_database_setup remain UNCHANGED) ...
...
def logged_request(self, method: str, url: str, **kwargs: Any) -> Any:
    ...
    'self' refers to the Session instance.
    """
    full_url = url
    ...
    # Headers
    # Combine session headers (self.headers) and request-specific headers (kwargs.get('headers', {}))
...
# ... (All core inference and evaluation functions remain UNCHANGED) ...
...
def run_ollama_inference(client: Client, model: str, system_prompt: str, user_prompt: str) -> Tuple[str, float]:
    ...
    return generated_text, end_time - time.time()
...
def run_tests(client: Client, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i, test in enumerate(TEST_CASES_V2_ENHANCED):
        ...
        print("Running: WITHOUT V0.2 Spec")
        ...
        print(f"Gen Call WO Spec: {generated_call_wo} (Time: {time_wo:.3f}s, Score: {score_wo:.1f})")
        ...
        print(f"Gen Call W Spec:  {generated_call_w} (Time: {time_w:.3f}s, Score: {score_w:.1f})")
